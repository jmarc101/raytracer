...
void Parser::parseSupersampling() {
    scene.supersampling = lexer.getNumber();
}
...
void Parser::parseMaterial() {
    ...
    lexer.getString();
    bitmap_image m = lexer.getBitmap();
    ParamList params = lexer.getParamList(1, 4);
    scene.materials[name] = Material(m,params);
